...
public class SignInCaptureActivity extends AppCompatActivity implements ZXingScannerView.ResultHandler, ScanListener {
    ...
    private void showMessageOKCancel(String message, DialogInterface.OnClickListener okListener) {
        new AlertDialog.Builder(SignInCaptureActivity.this)
                .setMessage(message)
    ...
    public void handleResult(Result result) {
        ...
        builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                scannerView.resumeCameraPreview(SignInCaptureActivity.this);
            }
        });
...
//        Log.d("debug", SignInUseCases.user.getId().toString());
        ...
        this.result = result;
        scanOptionRemoteSource.getAllBookings((NormalUser) SignInUseCases.user, SignInCaptureActivity.this);
        // scanOptionUseCases.getSigninTimeSlot((NormalUser) SignInUseCases.user, SignInCaptureActivity.this);
        ...
        if (realTimeSlot == null) {
            ...
            if (realTimeSlot.getPlaceId() == (result.charAt(0) - '0')) {
                ...
                if (flag) {
                    Toast.makeText(this, "seat is correct", Toast.LENGTH_SHORT).show();
                    Log.d("debug","seat is correct");
                    realTimeSlot.setInTime((int) (System.currentTimeMillis() / 1000)); // update the sign in time;
                    realTimeSlot.setState(State.SIGNIN); // update the state
                    // dbTimeSlotManager.updateTimeSlot(t);

                        Map<String, Object> update = new HashMap<>();
                        update.put(realTimeSlot.getKey() + "/state", realTimeSlot.getState());
                        update.put(realTimeSlot.getKey() + "/inTime", realTimeSlot.getInTime());
