...
public class TempOutCaptureActivity extends AppCompatActivity implements ZXingScannerView.ResultHandler, ScanListener {
...
//        Log.d("debug", SignInUseCases.user.getId().toString());
        ...
        this.result = result;
        scanOptionRemoteSource.getAllBookings((NormalUser) SignInUseCases.user, TempOutCaptureActivity.this);
        ...
        if (realTimeSlot == null) {
            ...
            if (realTimeSlot.getPlaceId() == (result.charAt(0) - '0')) {
                ...
                if (flag) {
                    Toast.makeText(this, "seat is correct", Toast.LENGTH_SHORT).show();
                    Log.d("debug","seat is correct");
                    realTimeSlot.setTempLeaveTime((int) (System.currentTimeMillis() / 1000)); // update the sign in time;
                    realTimeSlot.setState(State.TMPLEAVE); // update the state
                    // dbTimeSlotManager.updateTimeSlot(t);

                    Map<String, Object> update = new HashMap<>();
                    update.put(realTimeSlot.getKey() + "/state", realTimeSlot.getState());
                    update.put(realTimeSlot.getKey() + "/tempLeaveTime", realTimeSlot.getTempLeaveTime());
                    ...
                    Toast.makeText(this,"Temp Sign Out successfully!", Toast.LENGTH_SHORT).show();
                    Intent intent = new Intent(this, WelcomeActivity.class);
                    intent.setFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
                    startActivity(intent);
